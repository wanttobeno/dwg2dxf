...
bool dxfRW::writeEntity(DRW_Entity *ent){
    ...
    if (version > DRW::AC1015 && ent->color24 >= 0) {
		/*
		一个 24 位颜色值，应按照值为 0 到 255 的字节进行处理。最低字节是蓝色值，
		中间字节是绿色值，第三个字节是红色值。最高字节始终为 0。该组码不能用于自定义图元本身的数据，
		因为该组码是为 AcDbEntity 类级别颜色数据和 AcDbEntity 类级别透明度数据保留的。
		*/
...
bool dxfRW::writeTextstyle(DRW_Textstyle *ent){
    ...
    if (version > DRW::AC1009) {
        writer->writeString(5, toHexStr(++entCount));// 写入句柄
    }
	...
	// 附着点,1 = 左上；2 = 中上；3 = 右上，九宫格
	writer->writeInt16(71, ent->genFlag);
	//成多行文字图元的字符的水平宽度。该值始终等于或小于组码 41 的值
...
bool dxfRW::writeMText(DRW_MText *ent){
    if (version > DRW::AC1009) {
		...
		wstring  wSrt;
		Transcode::UTF8_to_Unicode(text.c_str(),text.length(),wSrt);
